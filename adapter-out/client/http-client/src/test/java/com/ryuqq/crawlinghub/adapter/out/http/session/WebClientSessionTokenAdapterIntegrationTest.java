...
class WebClientSessionTokenAdapterIntegrationTest {
    ...
    void setUp() {
        ...
        httpProperties.setConnectTimeout(10);
        httpProperties.setRequestTimeout(30);

        SessionTokenProperties sessionProperties = new SessionTokenProperties();
        sessionProperties.setTargetUrl("https://m.web.mustit.co.kr/");
        sessionProperties.setSessionCookieName("token");
        sessionProperties.setDefaultSessionDurationHours(2);

        WebClientConfig config = new WebClientConfig(httpProperties);
        WebClient webClient = config.webClient();

        adapter = new WebClientSessionTokenAdapter(webClient, sessionProperties, Clock.systemUTC());
    }
