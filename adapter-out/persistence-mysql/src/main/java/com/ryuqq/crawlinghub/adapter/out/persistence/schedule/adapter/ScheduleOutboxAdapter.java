 ...
 */
@Component
public class ScheduleOutboxAdapter implements ScheduleOutboxPort {
     ...
     */
    @Override
    public java.util.Optional<ScheduleOutbox> findByIdemKey(String idemKey) {
    ...
    public ScheduleOutbox findLatestBySellerId(Long sellerId) {
        ...
        List<ScheduleOutboxEntity> outboxes =
    ...
    public void updateOpId(Long outboxId, String opId) {
        ...
        ScheduleOutboxEntity entity = repository.findById(outboxId)
                .orElseThrow(() -> new IllegalStateException(
                        "Outbox not found for ID: " + outboxId
                ));

        // Entity 필드 직접 업데이트 (JPA 변경 감지)
        entity = new ScheduleOutboxEntity(
                entity.getId(),
                opId,
                entity.getSellerId(),
                entity.getIdemKey(),
    ...
    public void markInProgress(String opId, String outcomeJson) {
        ...
        ScheduleOutboxEntity entity = repository.findByOpId(opId)
                .orElseThrow(() -> new IllegalStateException(
    ...
    public void markCompleted(String opId, String finalState) {
        ...
        ScheduleOutbox.OperationState entityState =
                ScheduleOutbox.OperationState.valueOf(finalState);
        // Entity 필드 직접 업데이트 (JPA 변경 감지)
        entity = new ScheduleOutboxEntity(
                entity.getId(),
                entity.getOpId(),
    ...
    public List<String> findPendingOperations(int limit) {
        ...
        List<ScheduleOutboxEntity> pendingOutboxes =
        ...
        return pendingOutboxes.stream()
                .map(ScheduleOutboxEntity::getOpId)
                .toList();
    }
    ...
    public List<String> findTimeoutOperations(long timeoutMillis, int limit) {
        ...
        List<ScheduleOutboxEntity> timedOutOutboxes =
     ...
     */
    public ScheduleOutbox getOperationEnvelope(String opId) {
        Objects.requireNonNull(opId, "opId must not be null");
    ...
    public List<ScheduleOutbox> findByWalStatePending() {
        List<ScheduleOutboxEntity> entities =
                repository.findByWalStateOrderByCreatedAtAsc(
                        ScheduleOutbox.WriteAheadState.PENDING,
