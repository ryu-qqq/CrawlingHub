...
DB_PASSWORD=[AWS Secrets Manager에서 관리]
```

**연결 문자열 예시**:
```
# AWS Secrets Manager를 통해 비밀번호 관리
# ECS Task Definition에서 secrets 블록으로 주입
```
...
connection = pymysql.connect(
    ...
    password=os.getenv('DB_PASSWORD'),  # Secrets Manager에서 가져온 값
    database='fileflow'
)
...
**1. `.env` 파일 생성**:
```bash
# .env (비밀번호는 Secrets Manager에서 가져오기)
DB_HOST=prod-shared-mysql.cfacertspqbw.ap-northeast-2.rds.amazonaws.com
DB_PORT=3306
DB_NAME=fileflow
DB_USER=fileflow_user
...
services:
  app:
    ...
    # 비밀번호는 .env 파일에서 관리 (Git에 커밋하지 않음)
    env_file:
      - .env
```
...
```bash
# 1. Secret 생성 (비밀번호는 Secrets Manager에서 확인)
kubectl create secret generic db-credentials \
  ...
  --from-literal=password='[Secrets Manager에서 가져온 비밀번호]'
...
```bash
# FileFlow 사용자로 연결 (비밀번호는 Secrets Manager에서 확인)
...
# 테스트 실행 (비밀번호는 환경 변수 또는 Secrets Manager에서 가져오기)
import os
test_connection(
    host=os.getenv('DB_HOST'),
    port=3306,
