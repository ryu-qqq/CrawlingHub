 ...
 *
 * <p>형식: {@code cron(minute hour day-of-month month day-of-week year)}</p>
 * <p>최소 실행 간격: 1시간</p>
 */
public record CronExpression(String value) {

    private static final Pattern AWS_CRON_PATTERN = Pattern.compile("^cron\\((?<body>.+)\\)$", Pattern.CASE_INSENSITIVE);
    ...
    private static String[] validateAwsEventBridgeFormat(String expression) {
        ...
        boolean hasInvalidField = Arrays.stream(fields)
            .anyMatch(field -> field.isBlank() || !FIELD_ALLOWED_PATTERN.matcher(field).matches());
