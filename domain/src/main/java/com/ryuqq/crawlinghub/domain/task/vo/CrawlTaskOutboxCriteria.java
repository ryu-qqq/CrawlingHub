 ...
 * @since 1.0.0
 */
public record CrawlTaskOutboxCriteria(OutboxStatus status, List<OutboxStatus> statuses, int limit) {
    ...
    public static CrawlTaskOutboxCriteria byStatus(OutboxStatus status, int limit) {
        return new CrawlTaskOutboxCriteria(status, null, limit);
    }
    ...
    public static CrawlTaskOutboxCriteria byStatuses(List<OutboxStatus> statuses, int limit) {
        return new CrawlTaskOutboxCriteria(null, statuses, limit);
    }
    ...
    public static CrawlTaskOutboxCriteria pendingOrFailed(int limit) {
        return new CrawlTaskOutboxCriteria(
                null, List.of(OutboxStatus.PENDING, OutboxStatus.FAILED), limit);
    }
