package com.ryuqq.crawlinghub.domain.crawler.vo;

/**
 * ScheduleId - CrawlingSchedule 식별자 Value Object
 *
 * <p>Auto-increment Long 기반 스케줄 고유 식별자</p>
 *
 * @param value Schedule ID (null이면 새로운 엔티티)
 */
public record ScheduleId(Long value) {

    /**
     * 새로운 Schedule ID 생성 (null)
     *
     * @return null을 가진 ScheduleId (새 엔티티 표시)
     */
    public static ScheduleId forNew() {
        return new ScheduleId(null);
    }
    /**
     * 기존 ID 값으로 ScheduleId 생성 (정적 팩토리 메서드)
     *
     * @param value ID 값
     * @return ScheduleId 인스턴스
     */
    public static ScheduleId of(Long value) {
        return new ScheduleId(value);
    }


    /**
     * 새로운 엔티티인지 확인
     *
     * @return value가 null이면 true (아직 DB에 저장되지 않음)
     */
    public boolean isNew() {
        return value == null;
    }
}
