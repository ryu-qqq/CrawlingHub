...
terraform {
  ...
  backend "s3" {
    ...
    key            = "crawlinghub/ecs-crawl-worker-stage/terraform.tfstate"
...
variable "image_tag" {
  description = "Docker image tag to deploy. Format: crawl-worker-stage-{build-number}-{git-sha}"
  type        = string
  default     = "crawl-worker-stage-1-initial"

  validation {
    condition     = can(regex("^crawl-worker-stage-[0-9]+-[a-zA-Z0-9]+$", var.image_tag))
    error_message = "Image tag must follow format: crawl-worker-stage-{build-number}-{git-sha} (e.g., crawl-worker-stage-1-abc1234)"
  }
}
...
# Deploy order: terraform/environments/stage/sqs → terraform/environments/stage/ecs-crawl-worker
data "aws_ssm_parameter" "sqs" {
  for_each = {
    crawl_task_queue_url          = "/${var.project_name}/sqs-stage/crawling-task-queue-url"
    crawl_task_dlq_url            = "/${var.project_name}/sqs-stage/crawling-task-dlq-url"
    eventbridge_trigger_queue_url = "/${var.project_name}/sqs-stage/eventbridge-trigger-queue-url"
