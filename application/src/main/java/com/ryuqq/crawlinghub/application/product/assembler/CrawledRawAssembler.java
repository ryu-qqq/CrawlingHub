 ...
 *
 * <ul>
 *   <li>MiniShopItem → CrawledRaw (JSON 직렬화)</li>
 *   <li>ProductDetailInfo → CrawledRaw (JSON 직렬화)</li>
 *   <li>ProductOption 목록 → CrawledRaw (JSON 직렬화)</li>
 *   <li>비즈니스 로직 없음 (단순 변환만)</li>
 * </ul>
 *
...
public class CrawledRawAssembler {
     ...
     */
    public List<CrawledRaw> toMiniShopRaws(long schedulerId, long sellerId, List<MiniShopItem> items) {
     ...
     * @return CrawledRaw (실패 시 null)
     */
    public CrawledRaw toOptionRaw(long schedulerId, long sellerId, long itemNo, List<ProductOption> options) {
     ...
     */
    private CrawledRaw toRaw(long schedulerId, long sellerId, long itemNo, CrawlType crawlType, Object data) {
        try {
            String json = objectMapper.writeValueAsString(data);
            return CrawledRaw.create(schedulerId, sellerId, itemNo, crawlType, json);
        } catch (JsonProcessingException e) {
            log.error("JSON 변환 실패: type={}, itemNo={}, error={}", crawlType, itemNo, e.getMessage());
            return null;
        }
    }
}
