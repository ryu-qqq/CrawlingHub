...
public class ListSellersService implements ListSellersUseCase {
    ...
    public PageResponse<SellerSummaryResponse> listSellers(ListSellersQuery query) {
        ...
        List<SellerSummaryResponse> content = sellers.stream()
            .map(seller -> {
                int totalSchedulerCount = schedulerQueryPort.countTotalSchedulersBySellerId(
                    seller.getSellerId().value()
                );
                return sellerAssembler.toSellerSummaryResponse(seller, totalSchedulerCount);
            })
