...
public interface SchedulerQueryPort {
    ...
    Optional<CrawlingScheduler> findBySellerIdAndSchedulerName(SellerId sellerId, String schedulerName);

    List<CrawlingScheduler> findBySellerIdAndStatus(SellerId sellerId, SchedulerStatus status);

    PageResult<CrawlingScheduler> findAllBySellerIdAndStatus(SellerId sellerId, SchedulerStatus status, int page, int size);
