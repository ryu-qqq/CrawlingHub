...
import com.ryuqq.crawlinghub.application.crawl.processing.port.out.NotificationPort;
...
public class HandleTaskFailureService implements HandleTaskFailureUseCase {
    ...
    public void execute(TaskFailureCommand command) {
        ...
        // 3. 상태에 따라 분기 처리
        if (task.hasStatus(com.ryuqq.crawlinghub.domain.crawl.task.TaskStatus.RETRY)) {
