package com.ryuqq.crawlinghub.application.image.port.out.messaging;

import com.ryuqq.crawlinghub.domain.product.aggregate.ProductImageOutbox;
import com.ryuqq.crawlinghub.domain.product.event.ImageUploadRequestedEvent;

/**
 * ProductImage 메시지 발행 Port (Port Out - Messaging)
 *
 ...
 *
 * <ul>
 *   <li>{@link #publishFromEvent(ImageUploadRequestedEvent)}: 트랜잭션 커밋 후 이벤트 리스너에서 호출
 *   <li>{@link #publishFromOutbox(ProductImageOutbox)}: 재시도 스케줄러에서 아웃박스 기반 호출
 * </ul>
 *
 ...
 * @since 1.0.0
 */
public interface ProductImageMessagePort {

    /**
     * ProductImageOutbox 기반 메시지 발행 (일반 발행)
     *
     * <p>Transaction commit 후 호출되어야 함 (afterCommit)
     *
     * @param outbox 발행할 ProductImageOutbox
     */
    void publish(ProductImageOutbox outbox);
     ...
     */
    void publishFromEvent(ImageUploadRequestedEvent event);
     ...
     */
    void publishFromOutbox(ProductImageOutbox outbox);
}
