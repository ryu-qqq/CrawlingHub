...
public class TransactionSynchronizationAdapter {
    ...
    public void processOutboxEvents() {
        List<OutboxEvent> pendingEvents = outboxEventQueryPort.findByStatusAndRetryCountLessThan(
    ...
    private void processEvent(OutboxEvent event) {
        ...
        switch (eventType) {
            case SCHEDULER_CREATED:
                SchedulerCreatedPayload createdPayload = parsePayload(event.payload(), SchedulerCreatedPayload.class);
                eventBridgeClientPort.createRule(ruleName, createdPayload.cronExpression(), createdPayload.target());
                break;
            case SCHEDULER_UPDATED:
                SchedulerUpdatedPayload updatedPayload = parsePayload(event.payload(), SchedulerUpdatedPayload.class);
        ...
        OutboxEvent published = OutboxEvent.of(
            event.eventId(),
            event.eventType(),
            event.schedulerId(),
            event.payload(),
    ...
    private void handleFailure(OutboxEvent event, Exception e) {
        ...
        OutboxStatus newStatus = newRetryCount >= event.maxRetries() 
            ? OutboxStatus.FAILED 
            : OutboxStatus.PENDING;

        OutboxEvent failed = OutboxEvent.of(
            event.eventId(),
            event.eventType(),
            event.schedulerId(),
            event.payload(),
