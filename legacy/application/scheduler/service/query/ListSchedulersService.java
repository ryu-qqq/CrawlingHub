...
import com.ryuqq.crawlinghub.application.schedule.port.in.query.ListSchedulersUseCase;
...
public class ListSchedulersService implements ListSchedulersUseCase {
    ...
    public PageResult<SchedulerSummaryResponse> execute(ListSchedulersQuery query) {
        PageResult<CrawlingScheduler> pageResult =
                schedulerQueryPort.findAllBySellerIdAndStatus(
                        SellerId.of(query.sellerId()), query.status(), query.page(), query.size());

        List<SchedulerSummaryResponse> content =
