...
import com.ryuqq.crawlinghub.application.schedule.port.in.query.GetSchedulerHistoryUseCase;
...
public class GetSchedulerHistoryService implements GetSchedulerHistoryUseCase {
    ...
    public PageResult<SchedulerHistoryResponse> execute(GetSchedulerHistoryQuery query) {
        PageResult<SchedulerHistory> pageResult =
                schedulerHistoryQueryPort.findBySchedulerId(
                        SchedulerId.of(query.schedulerId()), query.page(), query.size());

        List<SchedulerHistoryResponse> content =
