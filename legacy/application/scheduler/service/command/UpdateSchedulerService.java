...
import com.ryuqq.crawlinghub.application.schedule.port.in.command.UpdateSchedulerUseCase;
...
public class UpdateSchedulerService implements UpdateSchedulerUseCase {
    ...
    public SchedulerResponse execute(UpdateSchedulerCommand command) {
        CrawlingScheduler scheduler =
                schedulerQueryPort
                        .findById(SchedulerId.of(command.schedulerId()))
        ...
        SchedulerStatus updatedStatus =
                command.status() != null ? command.status() : scheduler.getStatus();
